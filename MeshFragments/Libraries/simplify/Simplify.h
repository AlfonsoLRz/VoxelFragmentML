...
namespace Simplify
{
	...
	struct Triangle { int v[3]; double err[4]; int deleted, dirty, constant; double threshold; vec3f n; };
	...
	void simplify_mesh(int target_count, double agressiveness = 7, bool verbose = false)
	{
		...
		// ready
		auto timeEnd = ChronoUtilities::getDuration();
		if (verbose)
	...
	//                 more iterations yield higher quality
	//
	void simplify_mesh_curvature(int target_count, double agressiveness = 7, bool verbose = false)
	{
		// init
		if (verbose)
			printf("%s - start\n", __FUNCTION__);
		...
		loop(iteration, 0, 100)
		{
			...
			if (iteration % 5 == 0)
			{
				...
				loopi(0, triangles.size())
				{
					//double curvature = glm::max(
					//	glm::max(glm::abs(triangles[i].n.dot(vec3f(0, 1, 0))), glm::abs(triangles[i].n.dot(vec3f(1, 0, 0)))),
					//	glm::abs(triangles[i].n.dot(vec3f(0, 0, 1)))
					//);
					double y = triangles[i].n.y;
					double abs_y = glm::abs(0.5f - glm::abs(triangles[i].n.y));
					triangles[i].threshold = agressiveness - (0.5f - abs_y) * 20.0;

					//std::cout << y << ", " << abs_y << ", " << triangles[i].threshold << std::endl;
					//constant += triangles[i].constant;
				}

				// Sort triangles according to threshold
				std::sort(triangles.begin(), triangles.end(), [](const Triangle& a, const Triangle& b) { return a.threshold > b.threshold; });

				//std::cout << "constant " << constant << " from " << triangles.size() << std::endl;
			}
			...
			{
				Triangle& t = triangles[i];
				if (t.err[3] > t.threshold) continue;
				if (t.deleted) continue;
				if (t.dirty) continue;
