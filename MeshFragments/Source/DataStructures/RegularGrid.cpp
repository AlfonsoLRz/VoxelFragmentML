...
void RegularGrid::queryCluster(const std::vector<Model3D::VertexGPUData>& vertices, const std::vector<Model3D::FaceGPUData>& faces, std::vector<float>& clusterIdx)
{
	...
	while (numProcessedFaces < faces.size())
	{
		...
		countVoxelTriangle->bindBuffers(std::vector<GLuint>{ vertexSSBO, faceSSBO, gridSSBO, countSSBO, noiseSSBO });
		...
		pickVoxelTriangle->bindBuffers(std::vector<GLuint>{ vertexSSBO, faceSSBO, countSSBO, clusterSSBO });
	...
	GLuint buffers[] = { countSSBO, vertexSSBO, gridSSBO, noiseSSBO };
	glDeleteBuffers(sizeof(buffers) / sizeof(GLuint), buffers);
