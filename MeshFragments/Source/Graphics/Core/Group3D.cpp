...
bool Group3D::hasCollided(Group3D* group)
{
	//_triangleSet->clear();

	//ChronoUtilities::initChrono();
	//
	//ComputeShader* meshBVHCollisionShader	= ShaderList::getInstance()->getComputeShader(RendEnum::MESH_BVH_COLLISION);
	//const GLuint nullCount					= 0;
	//const GLuint faceIndexSSBO				= ComputeShader::setWriteBuffer(GLuint(), _staticGPUData->_numTriangles, GL_DYNAMIC_DRAW);
	//const GLuint numCollisionsSSBO			= ComputeShader::setReadBuffer(&nullCount, 1, GL_DYNAMIC_DRAW);
	//const GLuint numGroups					= ComputeShader::getNumGroups(_staticGPUData->_numTriangles * 3);

	//meshBVHCollisionShader->bindBuffers(std::vector<GLuint> { 
	//	_staticGPUData->_groupGeometrySSBO, _staticGPUData->_groupTopologySSBO, _staticGPUData->_groupMeshSSBO,
	//	group->_staticGPUData->_clusterSSBO, group->_staticGPUData->_groupGeometrySSBO, group->_staticGPUData->_groupTopologySSBO, group->_staticGPUData->_groupMeshSSBO,
	//	faceIndexSSBO, numCollisionsSSBO
	//});
	//meshBVHCollisionShader->use();
	//meshBVHCollisionShader->setUniform("mModelSource", _modelMatrix);
	//meshBVHCollisionShader->setUniform("mModelDest", glm::inverse(group->_modelMatrix));
	//meshBVHCollisionShader->setUniform("numClusters", group->_staticGPUData->_numTriangles * 2 - 1);
	//meshBVHCollisionShader->setUniform("numTriangles", _staticGPUData->_numTriangles);
	//meshBVHCollisionShader->execute(numGroups, 1, 1, ComputeShader::getMaxGroupSize(), 1, 1);

	//GLuint numCollisions = *ComputeShader::readData(numCollisionsSSBO, GLuint());
	//const long long measuredMs = ChronoUtilities::getDuration();
	//
	//GLuint* faces = ComputeShader::readData(faceIndexSSBO, GLuint());
	//std::vector<GLuint> faceBuffer = std::vector<GLuint>(faces, faces + numCollisions);
	//std::set<GLuint> faceSet (faceBuffer.begin(), faceBuffer.end());
	//faceBuffer.assign(faceSet.begin(), faceSet.end());

	//std::cout << "Response time per collided face: " << measuredMs / float(faceBuffer.size()) << std::endl;

	//_triangleSet->insertFaces(&_groupData->_geometry, &_groupData->_triangleMesh, faceBuffer.data(), faceBuffer.size());
	//_triangleSet->load();
...
bool Group3D::hit(const RayGPUData& ray, float& distance, GLuint& modelCompID)
{
	// Move ray to our coordinate system
	RayGPUData copyRay = ray;
	mat4 inverseMatrix = glm::inverse(_modelMatrix);
	
	copyRay._origin			= vec3(inverseMatrix * vec4(copyRay._origin, 1.0f));
	copyRay._destination	= vec3(inverseMatrix * vec4(copyRay._destination, 1.0f));
	copyRay._direction		= vec3(inverseMatrix * vec4(copyRay._direction, 0.0f));
	
	ComputeShader* bvhCollisionShader	= ShaderList::getInstance()->getComputeShader(RendEnum::BVH_COLLISION);
	const GLuint raySSBO				= ComputeShader::setReadBuffer(&copyRay, 1, GL_STATIC_DRAW);
	const GLuint collisionSSBO			= ComputeShader::setWriteBuffer(Model3D::TriangleCollisionGPUData(), 1, GL_DYNAMIC_DRAW);

	for (StaticGPUData* staticData: _staticGPUData)
	{
		bvhCollisionShader->bindBuffers(std::vector<GLuint> { staticData->_clusterSSBO, staticData->_groupGeometrySSBO, staticData->_groupTopologySSBO, staticData->_groupMeshSSBO, raySSBO, collisionSSBO });
		bvhCollisionShader->use();
		bvhCollisionShader->setUniform("numClusters", staticData->_numClusters);
		bvhCollisionShader->setUniform("numRays", GLuint(1));
		bvhCollisionShader->execute(1, 1, 1, 1, 1, 1);

		TriangleCollisionGPUData* collisionData = ComputeShader::readData(collisionSSBO, Model3D::TriangleCollisionGPUData());

		if (collisionData[0]._faceIndex != 0xFFFFFFF)
		{
			distance = glm::distance(ray._origin, collisionData[0]._point);
			modelCompID = collisionData[0]._modelCompID;

			return true;
		}
	}

	glDeleteBuffers(1, &raySSBO);
	glDeleteBuffers(1, &collisionSSBO);
